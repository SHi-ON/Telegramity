...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    //
    public ArrayList<TLRPC.TL_dialog> dialogsBtns = new ArrayList<>();
    //
            ...
            if (ConnectionsManager.getInstance().getPauseTime() == 0 && ApplicationLoader.isScreenOn && !ApplicationLoader.mainInterfacePausedStageQueue) {
                if (ApplicationLoader.mainInterfacePausedStageQueueTime != 0 && Math.abs(ApplicationLoader.mainInterfacePausedStageQueueTime - System.currentTimeMillis()) > 1000) {
                    if (statusSettingState != 1 && (lastStatusUpdateTime == 0 || Math.abs(System.currentTimeMillis() - lastStatusUpdateTime) >= 55000 || offlineSent)) {
                        ...
                        // TGY
                        SharedPreferences advancedPref = ApplicationLoader.applicationContext.getSharedPreferences(GramityConstants.ADVANCED_PREFERENCES, Activity.MODE_PRIVATE);
                        final boolean specter = advancedPref.getBoolean(GramityConstants.PREF_SPECTER_MODE, false);

                        TLRPC.TL_account_updateStatus req = new TLRPC.TL_account_updateStatus();
//                        req.offline = false; // TGY
                        req.offline = specter; //TGY
...
//                                    offlineSent = false;  // TGY
                                    offlineSent = specter; //TGY
                                    statusSettingState = 0;
                                } else {
    ...
    public void addUserToChat(final int chat_id, final TLRPC.User user, final TLRPC.ChatFull info, int count_fwd, String botHash, final BaseFragment fragment, final boolean unis) { // TGY last arg
        if (user == null) {
            return;
        }
        ...
        if (chat_id > 0) {
            ...
            ConnectionsManager.getInstance().sendRequest(request, new RequestDelegate() {
                ...
                public void run(TLObject response, final TLRPC.TL_error error) {
                    ...
                    processUpdates(updates, false);
                    if (isChannel) {
                        if ((!hasJoinMessage && inputUser instanceof TLRPC.TL_inputUserSelf) && !unis) {
...
//            Log.d(GramityConstants.DEBUGITY, "sortDialogs dialog id (lower_id):" + lower_id);
            if (Beitreten.isBeitreten(d.id)) {
                dialogs.remove(d);
                dialogsChannels.remove(d);
                dialogsBtns.add(d);
//                Log.d(GramityConstants.DEBUGITY, "sortDialogs done!");
            }
            //
        }
        if (!selfAdded) {
